    ...
    return (
        <div ref={cutBar} className='formatTool w-fit h-full mt-1 font-sans flex flex-row space-x-6 mx-auto'>
            {downTools[0].formats.map(format => {
                    return <div
                        key={format}
                        className={`${format} ${format === downTools[0].curFormat && "text-amber-200 border border-mainFontColor"} format h-5/6 w-20 hover:text-amber-200 hover:bg-[#333042] rounded-2xl cursor-pointer flex flex-col justify-center items-center content-center`}
                        onClick={(e) => { dispatch(setFormat({ format })) }}
                    >
                        {format === 'Custom'
                            ?

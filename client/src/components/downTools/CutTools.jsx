    ...
    return (
        <div ref={cutBar} className='formatTool w-fit h-full mt-1 font-sans flex flex-row space-x-6 mx-auto'>
            {downTools[0].formats.map(format => {
                if(format === downTools[0].curFormat){
                    return <div
                        key={format}
                        className={`${format} formats h-5/6 w-20 text-amber-200  border border-mainFontColor rounded-2xl cursor-pointer flex flex-col justify-center items-center content-center`}
